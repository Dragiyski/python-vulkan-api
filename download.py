...
def main():
    ...
    for metadata_file in metadata_files:
        source_url = urllib.parse.urljoin(repository_url, metadata_file)
        target_file = target_dir.joinpath(metadata_file)
        status, reason, _ = fetch_file(target_file, source_url)
        log(f'{source_url} : {target_file} : {status} {reason}')
