...
def get_data(*files):
    ...
    logger = logging.getLogger('vulkan.registry')
    roots = {}

    for file in files:
        xml = parse_xml(file, is_file=True)
        roots[str(file)] = xml
            ...
            for type_node in types_node.get_all('type'):
                ...
                if type_node.has_attribute('alias'):
                    ...
                    if category == 'bitmask':
                        ...
                        labels['bitmask'].add(name)
                    elif category == 'funcpointer':
                        labels['callback'].add(name)
        ...
        for enums_node in xml.get_all('enums'):
            ...
            if enums_node.has_attribute('name') and (enums_node.has_attribute('alias') or enums_node.has_attribute('type') and enums_node.get_attribute('type') in ['enum', 'bitmask']):
                ...
                if enums_node.has_attribute('alias'):
                    labels['alias'].add(enum_name)
                else:
                    labels['enum'].add(enum_name)
                    labels['type'].add(enum_name)
            ...
            for require_node in feature_node.get_all('require'):
                ...
                for node_type in ['type', 'enum', 'command']:
                    for node in require_node.get_all(node_type):
                        ...
                        if node.has_attribute('alias') or name in labels['alias']:
                            labels['alias'].add(name)
                            if node_type == 'enum':
    ...
    return SimpleNamespace(
        ...
        labels=labels,
        tags=tags,
        enum_bit_map=enum_bit_map,
        bit_enum_map=bit_enum_map,
