                ...
                for node_type in ['type', 'enum', 'command']:
                    for node in require_node.get_all(node_type):
                        ...
                        if name in nodes:
                            ...
                            logger.warn('Feature "%s" includes name "%s" that was not previously defined.' % (feature_node.get_attribute('name'), name))
                    ...
                    for node_type in ['type', 'enum', 'command']:
                        for node in require_node.get_all(node_type):
                            ...
                            if name in nodes:
                                ...
                                logger.warn('Extension "%s" includes name "%s" that was not previously defined.' % (extension_node.get_attribute('name'), name))
__init__()
del __init__
