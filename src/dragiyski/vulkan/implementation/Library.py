...
def load_get_instance_proc_address():
...
class Library:
    ...
    class Loader:
        def __init__(self, vkGetInstanceProcAddr = None):
            if vkGetInstanceProcAddr is None:
                vkGetInstanceProcAddr = load_get_instance_proc_address()
            if not callable(vkGetInstanceProcAddr):
                raise ValueError('Parameter "vkGetInstanceProcAddr" must be callable')
    ...
    def enumerate_instance_version(self, use_api = False):
        ...
        status_check(self._loader.vkEnumerateInstanceVersion(version))
        return version
    ...
    def enumerate_instance_layer_properties(self):
        ...
        c_array = []
        while count.value > 0:
            c_array = (vkEnumerateInstanceLayerProperties.argtypes[1]._type_ * count.value)()
            try:
