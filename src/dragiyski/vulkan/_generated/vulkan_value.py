...
def VK_API_VERSION_MAJOR(version):
    return ctypes.c_uint32(version).value >> 22 & 127

def VK_API_VERSION_MINOR(version):
    return ctypes.c_uint32(version).value >> 12 & 1023

def VK_API_VERSION_PATCH(version):
    return ctypes.c_uint32(version).value & 4095

def VK_API_VERSION_VARIANT(version):
    return ctypes.c_uint32(version).value >> 29

def VK_MAKE_API_VERSION(variant, major, minor, patch):
    return ctypes.c_uint32(variant).value << 29 | ctypes.c_uint32(major).value << 22 | ctypes.c_uint32(minor).value << 12 | ctypes.c_uint32(patch).value

def VK_MAKE_VERSION(major, minor, patch):
    return ctypes.c_uint32(major).value << 22 | ctypes.c_uint32(minor).value << 12 | ctypes.c_uint32(patch).value

def VK_MAKE_VIDEO_STD_VERSION(major, minor, patch):
    return ctypes.c_uint32(major).value << 22 | ctypes.c_uint32(minor).value << 12 | ctypes.c_uint32(patch).value

def VK_VERSION_MAJOR(version):
    return ctypes.c_uint32(version).value >> 22
